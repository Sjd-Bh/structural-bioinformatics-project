...
foreach f [glob "./cleanPDB/*.pdb"] {
	...
	set BR_resid [[atomselect $i $sel] get {resname resid structure}]
	lappend GBR_resid $BR_resid
	#puts $GBR_resid
	set j [expr {1+$j}]
}

set GBR [lsort -unique -index 1 -integer [join $GBR_resid]]

set output [open GBR_resname.txt w]
puts $output $GBR_resid
close $output

set output1 [open GBR_str.txt w]
puts $output1 $GBR_str
close $output1